...
dim = 50;  % PCA reduce dimension to 2000 suggested by Joint Bayesian paper
ind = 0;
numGaussian = 2;  % number of guassian 
...
gmmModel = get_joint(intra_s, inter_s);
...
intra_distance = sim_jointBayesian_joint_mixture(pairlist_lfw.IntraPersonPair, data, gmmModel, intra_s);
fprintf('Done!\n');
...
extra_distance = sim_jointBayesian_joint_mixture(pairlist_lfw.ExtraPersonPair, data, gmmModel, intra_s);
fprintf('Done!\n');
...
save(sprintf('joint_mixture_intra_precision_dim_%d_numGaussian_%d', dim, numGaussian), 'intra_precision');
save(sprintf('joint_mixture_extra_precision_dim_%d_numGaussian_%d', dim, numGaussian), 'extra_precision');
fprintf('Done!\n');